...
- Буферизованный ответ: `top:TYPE:buffered:1`
...
| Параметр | Значение |
|----------|----------|
| Порог prefetch | `max(1800px, 2.75 × высота экрана)` |
| `nearEnd` | расстояние до низа < порога |
| `hasUserScrolled` | `scrollY > 72` |
| Дебаунс | `requestAnimationFrame` на scroll/resize |

При `nearEnd && hasMore && !isLoadingMore` → `loadNextPage()`.

После завершения загрузки, если пользователь всё ещё внизу, срабатывает повторный prefetch (второй `useEffect`).
...
показывать, если:
  режим ≠ search
  AND hasMore
  AND (isLoadingMore OR (nearEnd AND hasUserScrolled))
```
...
| `SCROLL_PREFETCH_VIEWPORTS` | 2.75 | Множитель высоты экрана для prefetch |
| `MIN_SCROLL_PREFETCH_PX` | 1800 | Нижняя граница prefetch |
